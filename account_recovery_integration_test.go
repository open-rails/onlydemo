...
func assertAccountRecovery(t *testing.T, app *blogTestServer, auth *appAuth, pool *pgxpool.Pool, successor blogTestUser) {
	...
	members := "/api/v1/channel/recoverableauthor-channel/members"
	...
	blogTestRequest(t, peer, http.MethodDelete, "/api/v1/user", user.token, map[string]any{"password": password}, http.StatusNoContent)
	...
	blogTestRequest(t, peer, http.MethodPost, "/api/v1/token", "", map[string]any{"grant_type": "refresh_token", "refresh_token": user.refreshToken}, http.StatusUnauthorized)
	blogTestRequest(t, peer, http.MethodPost, "/api/v1/password/login", "", map[string]any{"identifier": "recoverableauthor@example.com", "password": "incorrect"}, http.StatusUnauthorized)
	login := map[string]any{"identifier": "recoverableauthor@example.com", "password": password}
	raw := blogTestRequest(t, peer, http.MethodPost, "/api/v1/password/login", "", login, http.StatusConflict)
	var proof struct {
		Error struct {
			Metadata struct {
				Recovery struct {
	...
	blogTestRequest(t, peer, http.MethodGet, "/api/posts", recovery.Token, nil, http.StatusUnauthorized)
	blogTestRequest(t, peer, http.MethodPost, "/api/v1/account/recovery/confirm", "", map[string]any{"token": recovery.Token}, http.StatusNoContent)
	blogTestRequest(t, peer, http.MethodPost, "/api/v1/account/recovery/confirm", "", map[string]any{"token": recovery.Token}, http.StatusUnauthorized)
	var session struct {
		AccessToken string `json:"access_token"`
	}
	decodeBlogTestJSON(t, blogTestRequest(t, peer, http.MethodPost, "/api/v1/password/login", "", login, http.StatusOK), &session)
